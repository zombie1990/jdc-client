 ...
 *
 */
public class BlogHibernateDAO extends GenericHibernateDAO<Blog, String> implements
		BlogDAO {

	public List<Comment> getComments(Blog blog) {
		String query = "from Comment c where c.blog = :blog" + " order by c.postTime desc";
		@SuppressWarnings("unchecked")
		List<Comment> comments = getSession().createQuery(query)
												.setEntity("blog", blog)
												.list();
		return comments;
	}
	...
	public List<Blog> getBlogsByType(String type, int start, int end) {
		...
		@SuppressWarnings("unchecked")
		List<Blog> blogs = getSession().createQuery(query)
